        ...
        if (err){
            ...
            async.series([
                    ...
                    function(callback) {
                        ...
                        //console.log('socket chat joined', resultMessages);
...
var socketModule = {
    ...
    socketObj: function(server) {
        ...
        io.on('connection', function(socket){
            ...
            socket.on('join data', function(data){
                SocketOperations.sendMyActivities(data.userId, function(err, resActs){
                    if(err){
                        ...
                        log.info('user and socket in chat: ' + data.userId + ' ' + socket.id, resActs.length);
            ...
            socket.on('system_push', function(data){
                log.info('system_push', data.ids, data.message);
                if(!common.isEmpty(data.ids)){
                    Notify.getDeviceIds(data.ids, function(err, udIds){
                        if(err){ console.error(err);}
                        else{
                            console.log('send system_push', udIds);
                            Pusher.sendPush(udIds, data.message);
                        }
    ...
    sendToCreator: function(userId, noSoloId, noSoloName, chatId, message, pushMessage, isCreator){
        Notify.chatBoxToOne(userId, function(err, socketId){
            if(err){ log.error(err); }
            else{
                //socketId, chatId, message, userId, userName, creatorId, pushMessage,messageType, imageUrl, tbNlImageUrl, messageTime
                sendMessageToCreator(socketId, chatId, message, noSoloId, noSoloName, userId, pushMessage, null, null, null, null, isCreator);
            }
        })
