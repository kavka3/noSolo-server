...
function joinChats(userId, socket, resActs){
...
function addToChat(socketId, chatId, userId){
    ...
    if(!common.isEmpty(socket)){
        ...
        socket.join(chatId);
        //socket.emit('new chat added', {result: 'success', data:{ chatId: chatId, messages: [] } });
        //don't need it in a new version see history
            ...
            socket.on('join data', function(data){
                SocketOperations.sendMyActivities(data.userId, function(err, resActs){
                    if(err){
                        ...
                        joinChats(data.userId, socket, resActs);
