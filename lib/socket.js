...
var socketModule = {
    ...
    chatClosed: function(chatId, userIds, creatorId, title){
        Notify.chatBoxToGroup(userIds, function(err, socketIds){
            if(err){log.error(err); }
            else{
                io.to(chatId).emit('chat closed', {result: 'success', data:{ chatId: chatId, messages: null, creatorId: creatorId, activityTitle: title } });
