...
//checked
function sendMessageToChat(userId, userName, chatId, message, messageId, isNtf, notForCreator, messageForPush){
        ...
        ChatManager.createMessage(userId, userName, chatId, message, msgIdChecked, notForCreator, function(err, resultMessage){
            if(err){
                ...
                if(isNtf && !messageForPush){ sendNtfToChat(resultMessage); }
                    ...
                    var pushMessage = ChatManager.createPushMessage(userId, userName, chatId, messageForPush);
...
var socketModule = {
    ...
    sendMyActivityLeave: function(userId, activityId){
        Notify.chatBoxToOne(userId, function(err, socketId){
                if(err){ log.error(err); }
                else{
                    log.info('sendMyActivityLeave', userId, socketId);
                    io.to(socketId).emit('my activities leave', {result: 'success', data: activityId});
                }
        })
    },
    ...
    removeFromChat: function(userId, chatId){
        Notify.chatBoxToOne(userId, function(err, socketId){
            if(err){ log.error('socket not found: ' + userId); }
            else{
                var socket = getSocket(socketId);
                if(!common.isEmpty(socket)){
                    socket.leave(chatId);
    ...
    sendToChat: function(userId, userName, chatId, message, sendNtf, notForCreator, pushMessage){
        sendMessageToChat(userId, userName, chatId, message, sendNtf, notForCreator, false, pushMessage);
    },
    ...
    sendToCreator: function(userId, noSoloId, noSoloName, chatId, message, pushMessage){
        Notify.chatBoxToOne(userId, function(err, socketId){
            if(err){ log.error(err); }
                ...
                sendMessageToCreator(socketId, chatId, message, noSoloId, noSoloName, userId, pushMessage);
            }
        })
    ...
    sendNewMember: function(noSoloId, nosoloName, chatId, message, userId, messageForPush){
        sendMessageToChat(noSoloId, nosoloName, chatId, message, false, false,
            { notForCreator: false, joinedUser: userId }, messageForPush);
    },
