            ...
            function(callback){
                Activity.findById(message.chatId, function(err, resAct){
                    if(err){ callback(err); }
                        ...
                        if(!common.isEmpty(resAct)){ callback(null, resAct.joinedUsers) }
...
function sendMessageToChat(userId, userName, chatId, message, messageId, isNtf, notForCreator){
    ...
    if(common.isEmpty(io['nsps']['/']['sockets'])){ log.error('Socket is empty'); }
        ...
        ChatManager.createMessage(userId, userName, chatId, message, messageId, notForCreator, function(err, resultMessage){
            if(err){
                log.error(err);
...
function sendMessageToCreator(socketId, chatId, message, userId, userName, creatorId){
    if(common.isEmpty(io['nsps']['/']['sockets'])){ log.error('Socket is empty'); }
        ...
        ChatManager.createMessage(userId, userName, chatId, message, messageId, { notForCreator : false, activityCreator: creatorId, notForOthers: true }, function(err, resultMessage){
            if(err){
                log.error(err);
                io.to(chatId).emit('error message', err.message);
            }
...
var socketModule = {
    ...
    socketObj: function(server) {
        ...
        io.on('connection', function(socket){
            ...
            socket.on('new message', function(data){
                log.info('new message: ' + data.message);
    ...
    sendToOthers: function(message, chatId, activityCreator){
        sendMessageToChat('100009647204771', 'noSolo', chatId, message, false,
