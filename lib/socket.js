...
module.exports = {
    socketObj: function(server) {
        ...
        io.on('connection', function(socket){
            ...
            socket.on('system_push', function(data){
                if(!common.isEmpty(data.ids)){
                    socketManager.getDeviceIds(data.ids, function(err, udIds){
                        if(err){ console.error(err);}
                        else{ Pusher.sendPush(udIds, data.message); }
                    })
                }
            });

            socket.on('system_block_users', function(data){
                SocketOperations.blockedUser(data, socket);
            })
        });
    ...
    sendMyActivityRepublish: function(activityId, response){
        io.to(activityId).emit('my activities republish', response);
        var msg = "Activity Republish - " + response.data.creator.surname + ", " +  response.data.title;
        socketManager.getDeviceIds(response.data.pendingUsers, function(err, udIds){
            if(err){ console.error(err);}
            else{ Pusher.sendPush(udIds, msg); }
        })
    },
    ...
    sendUserBlock: function(userId){
        socketManager.chatBoxToOne(userId, function(err, socketId){
            if(err){ console.error(err); }
            else if(socketId){
                io.to(socketId).emit('you are blocked', { userId: userId });
            }
        })
    }
};
