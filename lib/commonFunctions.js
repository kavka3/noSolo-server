...
function getInterval(reminderTime, timeStart){
    ...
    start.setHours(0,1,0,0);//setUTCHours(-24)
...
var common = {
    ...
    setMultipleReminder: function(user, createdActivity, callbackDone){
        ...
        var iterator = function(period, callbackI){
            ...
            var finish = new Date( interval.finish);
            finish.setHours(23,59,59,0);
            start.setHours(0,0,1,0);
            async.waterfall([
                function(callback){
                    Reminder.findOne({reminderTime: {$gte: start, $lt: finish}}, function(err, resReminder){
                        if(err){ callback(err); }
        ...
        async.eachSeries(user.settings.multipleReminders, iterator, function(err, result){
            ...
            else{ callbackDone(null, createdActivity, user); }
        })
