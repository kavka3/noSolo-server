...
var common = {
    ...
    //return array that not include intersections with arr2
    ...
    getTimeDifference: function(time){
        ...
        var targetUtc = moment.utc(date);
        var targetLocal = timeZone(time).tz('Israel');
        var result = {
            sameWeek: isSameWeek(now, targetLocal),
    ...
    convertTags: function(tagsArr, userLang){
        var res = [];
        if(!underscore.isEmpty(tagsArr)){
            tagsArr.forEach(function(tagObj){
                //console.log('Tag Dictionary', tagObj['tagDictionary']);
                var tagName = '';
                tagObj['tagDictionary'].forEach(function(e){
                    if(e[userLang]){
                        tagName = e[userLang];
                    }
                })
                var tag = {
                    _title: tagObj['_title'],
                    imageUrl: tagObj['imageUrl'],
                    name: tagName,
                    tagCategory: tagObj['tagCategory']
                };
                res.push(tag);
            });
        }
...
//common.getTimeDifference('2016-03-29T20:00:00.000Z');